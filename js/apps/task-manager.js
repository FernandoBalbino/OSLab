(function createTaskManagerApp(global) {
  ...
  function render(record) {
    ...
    const workspace = record.taskNav === "processes" ? `<div class="taskmanager-commandbar"><h2>Processos</h2><div><button type="button" data-task-action="run"><img src="assets/icons/taskmanager/run.png" alt="" /><span>Executar nova tarefa</span></button><button type="button" data-task-action="end" ${selected ? "" : "disabled"}><img src="assets/icons/context/delete.png" alt="" /><span>Finalizar tarefa</span></button><button type="button" data-task-action="efficiency" ${selected && !selected.protected ? "" : "disabled"}><img src="assets/icons/taskmanager/performance.png" alt="" /><span>Modo de eficiência</span></button><button type="button" data-task-action="refresh" aria-label="Atualizar"><img src="assets/icons/taskmanager/more.png" alt="" /></button></div></div>${processTable(record)}` : `<section class="taskmanager-info-page"><img src="${nav.find(([id]) => id === record.taskNav)?.[2] || "assets/icons/taskmanager/settings.png"}" alt="" /><h2>${safe(record.address.textContent)}</h2><p>Esta seção é informativa nesta versão do OSLab. A página Processos está totalmente disponível.</p><button type="button" data-task-nav="processes">Voltar para Processos</button></section>`;
