(function createAssistantRobot(global) {
  ...
  function render() {
    ...
    root.innerHTML = `<div class="robot-speech"><header><span><small>${phaseLabel(session.phase)}</small><strong>${safe(exercise.title)}</strong></span><button type="button" data-robot-toggle aria-label="Recolher ajudante">−</button></header><p>${safe(session.speech)}</p>${completed ? `<section class="robot-result"><span><strong>Causa</strong>${safe(session.result.cause)}</span><span><strong>Ferramenta</strong>${safe(session.result.tool)}</span><span><strong>Dica</strong>${session.result.hintUsed ? "Utilizada" : "Não utilizada"}</span></section>` : ""}<footer>${completed ? `<button type="button" data-robot-action="return">Voltar</button><button type="button" data-robot-action="repeat">Refazer</button><button class="is-primary" type="button" data-robot-action="next">Próximo</button>` : `<button type="button" data-robot-action="hint" ${session.hintUsed ? "disabled" : ""}>${session.hintUsed ? "Dica utilizada" : "Dica"}</button><button type="button" data-robot-action="repeat-speech">Repetir fala</button><button class="is-primary" type="button" data-robot-action="test">Testar novamente</button><button type="button" data-robot-action="restart">Reiniciar</button><button type="button" data-robot-action="exit">Sair</button>`}</footer></div><div class="robot-character">${robotSvg()}</div>`;
